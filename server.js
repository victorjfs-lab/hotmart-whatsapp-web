...
async function resetWhatsApp() {
  ...
  whatsappState.qrDataUrl = "";
  whatsappState.lastError = "Sessao limpa. Clique em iniciar para oerar outro QR Code.";
...
async function resolveWhatsAppRecipient(phone) {
  ...
  if (!normalized) throw new Erro("Telefone do comprador viazio.");
...
function buildContactSummary(jobs) {
  ...
  for (const job of jobs) {
    ...
    contact = grouped.get(job.saleId);
